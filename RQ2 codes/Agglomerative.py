...
def get_Agglomerative(X, y):
    ...
    tp, fp, tn, fn = compute_confusion_matrix(y_pred_res, y_train)
    acc, precision, recall, f1 = compute_indexes(tp, fp, tn, fn)
    accuracy = accuracy_score(y_train, y_pred_res)
    auc = roc_auc_score(y_train, y_pred_res)
    mcc = matthews_corrcoef(y_train, y_pred_res)
    return accuracy, f1, auc, mcc


def upper_sampling_Agglomerative(X, y):
    ...
    y_pred = AgglomerativeClustering(n_clusters=2).fit_predict(os_features)
    y_pred_res = np.array(y_pred)

    tp, fp, tn, fn = compute_confusion_matrix(y_pred_res, os_labels)
    acc, precision, recall, f1 = compute_indexes(tp, fp, tn, fn)
    accuracy = accuracy_score(os_labels, y_pred_res)
    auc = roc_auc_score(os_labels, y_pred_res)
    mcc = matthews_corrcoef(os_labels, y_pred_res)
    ...
    print(os_features_10.shape)

    y_pred_10 = AgglomerativeClustering(n_clusters=2).fit_predict(os_features_10)
    y_pred_res = np.array(y_pred_10)

    tp_10, fp_10, tn_10, fn_10 = compute_confusion_matrix(y_pred_res, os_labels_10)
    acc_10, precision_10, recall_10, f1_10 = compute_indexes(tp_10, fp_10, tn_10, fn_10)
    accuracy_10 = accuracy_score(os_labels_10, y_pred_res)
    auc_10 = roc_auc_score(os_labels_10, y_pred_res)
    mcc_10 = matthews_corrcoef(os_labels_10, y_pred_res)

    return accuracy, f1, auc, mcc, accuracy_10, f1_10, auc_10, mcc_10


def under_sampling_Agglomerative(X, y):
    y = np.bitwise_not(y) * 1  # 反转标签
    ...
    y_pred = AgglomerativeClustering(n_clusters=2).fit_predict(X_train_10)
    y_pred_res = [val_to_res(n) for n in y_pred]
    y_pred_res = np.array(y_pred_res)
...
for i in range(9,10):
    ...
    for epoch in range(10):
        ...
        acc_c, f1_c, auc_c, mcc_c, acc_c_10, f1_c_10, auc_c_10, mcc_c_10 = upper_sampling_Agglomerative(X_cov, y_cov)
        ...
        accuracy_i, f1_i, auc_i, mcc_i, acc_i_10, f1_i_10, auc_i_10, mcc_i_10 = upper_sampling_Agglomerative(X_input,
                                                                                                             y_input)
        ...
        accuracy_a_o, f1_a_o, auc_a_o, mcc_a_o, acc_o_10, f1_o_10, auc_o_10, mcc_o_10 = upper_sampling_Agglomerative(
            X_al_output,
            y_al_output)
        ...
        accuracy_cov_al_input, f1_c_a_i, auc_c_a_i, mcc_c_a_i, acc_a_10, f1_a_10, auc_a_10, mcc_a_10 = upper_sampling_Agglomerative(
            X_cov_al_input, y_cov_al_input)
        ...
        out_df.to_csv("./Coverage/10%Compare/Agglomerative/上/" + netname + "_" + dataname + "_" + str(i) + ".csv")
